  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
        isScrolled ? 'bg-white/90 backdrop-blur-md py-6 shadow-sm border-b border-zinc-100' : 'bg-transparent py-10'
      }`}
    >
      <div className="container mx-auto flex items-center justify-between px-6 lg:px-12 max-w-screen-2xl">
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="bg-white border-none w-full sm:w-[500px] flex flex-col justify-center items-center gap-12">
               <nav className="flex flex-col items-center gap-10">
                  ...
                  <Link to="/" className="text-5xl font-serif hover:text-zinc-400 transition-colors uppercase tracking-tight">{t('nav.newArrivals')}</Link>
        ...
        <nav className="hidden lg:flex items-center gap-12">
          <Link to="/products" className={`text-[10px] uppercase tracking-[0.5em] font-bold transition-all duration-500 font-serif ${isScrolled ? 'text-black hover:text-zinc-400' : 'text-white hover:text-white/60'}`}>
            {t('nav.collections')}
          </Link>
          <Link to="/" className={`text-[10px] uppercase tracking-[0.5em] font-bold transition-all duration-500 font-serif ${isScrolled ? 'text-black hover:text-zinc-400' : 'text-white hover:text-white/60'}`}>
            {t('nav.newArrivals')}
          </Link>
        </nav>
        ...
        <Link to="/" className="absolute left-1/2 transform -translate-x-1/2">
           <motion.span 
             ...
             className={`text-3xl md:text-5xl font-serif font-bold uppercase tracking-[0.8em] transition-all duration-700 ${isScrolled ? 'text-black' : 'text-white'}`}
           >
             JOYA
           </motion.span>
        </Link>
        ...
        <div className="flex items-center gap-8">
           <Button 
             ...
             className={`hidden md:flex items-center gap-3 text-[10px] font-bold tracking-[0.4em] transition-all duration-500 font-serif ${isScrolled ? 'text-black hover:text-zinc-400' : 'text-white hover:text-white/60'}`}
           >
           ...
           <Link to="/cart" className="relative group">
              <ShoppingCart className={`h-5 w-5 transition-all duration-500 ${isScrolled ? 'text-black group-hover:text-zinc-400' : 'text-white group-hover:text-white/60'}`} />
           </Link>

           <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="hover:bg-transparent group">
                  <User className={`h-5 w-5 transition-all duration-500 ${isScrolled ? 'text-black group-hover:text-zinc-400' : 'text-white group-hover:text-white/60'}`} />
                </Button>
              </DropdownMenuTrigger>
