  ...
  return (
    <div className="flex min-h-screen bg-white" dir={language === 'he' ? 'rtl' : 'ltr'}>
      ...
      <aside className="hidden lg:flex w-64 flex-col fixed inset-y-0 start-0 bg-black text-white border-e border-zinc-800">
        <div className="flex h-full flex-col justify-between">
          <div className="flex-1">
            <Link to="/admin" className="flex items-center justify-center h-24 border-b border-zinc-800 text-3xl font-serif font-bold uppercase tracking-[0.3em] text-white">
              JOYA
            </Link>
            <nav className="flex-1 space-y-2 px-4 py-10">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.link}
                  className="flex items-center gap-4 rounded-none px-4 py-4 text-zinc-400 transition-all hover:text-white hover:bg-zinc-900 uppercase text-[10px] tracking-widest font-bold"
                >
                  <item.icon className="h-4 w-4" />
                  {item.name}
                </Link>
          ...
          <div className="p-4 border-t border-zinc-800 pb-10">
            <Link
              to="/"
              className="flex items-center gap-4 rounded-none px-4 py-4 text-zinc-400 transition-all hover:text-white hover:bg-zinc-900 uppercase text-[10px] tracking-widest font-bold"
            >
